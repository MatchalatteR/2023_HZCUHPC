...
PCGReturn pcg_solve(const LduMatrix &ldu_matrix, double *source, double *x, int maxIter, double tolerance, double normfactor)
{
    ...
    printf("Initializing time cost: %.4lfs\n", pcg_end - pcg_start);

    gettimeofday(&tv, NULL);
    pcg_start = (double)(tv.tv_sec) + (double)(tv.tv_usec) * 1e-6;
    // AX = A * X
    ...
    gettimeofday(&tv, NULL);
    pcg_end = (double)(tv.tv_sec) + (double)(tv.tv_usec) * 1e-6;
    printf("solving time cost: %.4lfs\n", pcg_end - pcg_start);
